...
function Coin({ running, speed, result, onFinish }: { running: boolean; speed: number; result: Exclude<Result, null> | null; onFinish: (result: Exclude<Result, null>) => void }) {
  ...
  useEffect(() => {
    ...
    rb.setAngvel({ x: direction * (18 + Math.random() * 5), y: (Math.random() - 0.5) * 1.5, z: (Math.random() - 0.5) * 1.2 }, true)
    rb.wakeUp()
    setSettled(false)
  ...
  return (
    <>
      <RigidBody ref={body} type="dynamic" colliders={false} ccd canSleep restitution={0.28} friction={0.68} linearDamping={0.08} angularDamping={0.38} mass={0.25} position={[0, COIN_CENTER_Y, 0]} enabledRotations={[true, true, true]} gravityScale={1}>
        <CylinderCollider args={[COIN_HALF_THICKNESS, COIN_RADIUS]} restitution={0.28} friction={0.68} />
      </RigidBody>
      <group ref={visual} castShadow>
        <mesh castShadow receiveShadow><cylinderGeometry args={[COIN_RADIUS, COIN_RADIUS, COIN_HALF_THICKNESS * 2, 96, 8]} /><meshStandardMaterial color="#b67b22" metalness={0.96} roughness={0.19} /></mesh>
        <group position={[0, COIN_HALF_THICKNESS + 0.004, 0]}>
          <mesh rotation={[-Math.PI / 2, 0, 0]}><circleGeometry args={[0.86, 96]} /><meshStandardMaterial color="#f2c75b" metalness={0.92} roughness={0.16} side={THREE.FrontSide} /></mesh>
          <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.002, 0]}><torusGeometry args={[0.68, 0.055, 12, 96]} /><meshStandardMaterial color="#a96e17" metalness={0.95} roughness={0.2} /></mesh>
        ...
        <group position={[0, -COIN_HALF_THICKNESS - 0.004, 0]} rotation={[Math.PI, 0, 0]}>
          <mesh rotation={[-Math.PI / 2, 0, 0]}><circleGeometry args={[0.86, 96]} /><meshStandardMaterial color="#d59a2f" metalness={0.9} roughness={0.2} side={THREE.FrontSide} /></mesh>
          <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.002, 0]}><torusGeometry args={[0.68, 0.055, 12, 96]} /><meshStandardMaterial color="#8a5916" metalness={0.95} roughness={0.22} /></mesh>
...
function Arena({ running, speed, result, onFinish }: { running: boolean; speed: number; result: Exclude<Result, null> | null; onFinish: (result: Exclude<Result, null>) => void }) {
  return (
    <Physics gravity={[0, -12.5, 0]} timeStep={1 / (120 * speed)} interpolate updatePriority={-1}>
...
function Scene({ running, speed, result, onFinish }: { running: boolean; speed: number; result: Exclude<Result, null> | null; onFinish: (result: Exclude<Result, null>) => void }) {
  return (
    <Canvas shadows camera={{ position: [0, 4.8, 10], fov: 38 }} dpr={[1, 2]}>
      <color attach="background" args={['#06080c']} /><fog attach="fog" args={['#06080c', 11, 24]} />
      <ambientLight intensity={0.48} /><spotLight position={[4, 10, 5]} angle={0.3} penumbra={0.72} intensity={150} castShadow shadow-mapSize={[2048, 2048]} /><pointLight position={[-5, 4, 3]} intensity={42} color="#5478ff" /><pointLight position={[4, 2, -4]} intensity={20} color="#ffb52e" /><Environment preset="studio" />
      <RoundedBox args={[11, 0.45, 7]} radius={0.22} smoothness={6} position={[0, 0.68, 0]} receiveShadow><meshStandardMaterial color="#0e131a" metalness={0.62} roughness={0.32} /></RoundedBox>
      <mesh position={[0, 0.91, 0]} receiveShadow rotation={[-Math.PI / 2, 0, 0]}><planeGeometry args={[10.5, 6.5]} /><meshStandardMaterial color="#151a22" metalness={0.3} roughness={0.6} /></mesh>
...
function App() {
  ...
  const flip = useCallback(() => { if (!running) { setResult(Math.random() < 0.5 ? 'HEADS' : 'TAILS'); setRunning(true) } }, [running])
  ...
  return (
    <main className="app-shell">
      ...
      <section className="game-card"><div className="viewport"><Scene running={running} speed={speed} result={result} onFinish={finish} /></div>
        <div className="hud"><div className="result-box"><span>RESULT</span><strong>{result ?? (running ? 'FLIPPING…' : 'READY')}</strong></div><button className="flip-button" onClick={flip} disabled={running}>{running ? 'FLIPPING…' : 'FLIP COIN'}</button>
          <div className="speed-control"><div className="speed-label"><span>ANIMATION SPEED</span><strong>{speed}×</strong></div><input aria-label="Animation speed" type="range" min={BASE_SPEED} max={BASE_SPEED * 4} step={BASE_SPEED} value={speed} onChange={(e) => setSpeed(Number(e.target.value))} /><div className="speed-ticks"><span>{BASE_SPEED}× REALTIME</span><span>{BASE_SPEED * 4}× FAST</span></div></div>
        </div>
      </section>
